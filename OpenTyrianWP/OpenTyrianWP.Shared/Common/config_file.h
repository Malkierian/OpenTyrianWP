 ...
 * 
 * This struct allows for storing up to 15 characters (plus a terminating \c '\0') __inline.  For
...
{
	/*!
	 * \brief The __inline buffer for short strings.
	 */
	char short_buf[16];
 ...
 * \return the string
 */
static __inline string_t string_init( const char *s )
{
	assert(s != NULL);
 ...
 * \return the C-string
 */
static __inline char *to_cstr( string_t *string )
{
	assert(string != NULL);
 ...
 * \return the C-string
 */
static __inline const char *to_const_cstr( const string_t *string )
{
	assert(string != NULL);
	char is_long = STRING_LONG_TAG(*string);
	return is_long ?
 ...
 */
static __inline config_section_t *config_add_section( config_t *config, const char *type, const char *name)
{
	assert(type != NULL);
 ...
 */
static __inline config_option_t *config_set_option( config_section_t *section, const char *key, const char *value)
{
	assert(key != NULL);
 ...
 * \return the option; \c NULL if out of memory
 */
static __inline config_option_t *config_get_or_set_option( config_section_t *section, const char *key, const char *value )
{
	assert(key != NULL);
 ...
 * \return void
 */
static __inline void config_set_string_option( config_section_t *section, const char *key, const char *value )
{
	assert(key != NULL);
 ...
 * \return the option; \c NULL if out of memory
 */
static __inline config_option_t *config_set_value( config_option_t *option, const char *value )
{
 ...
 * \return the option; \c NULL if out of memory
 */
static __inline config_option_t *config_add_value( config_option_t *option, const char *value )
{
	assert(value != NULL);
 ...
 */
static __inline bool config_is_value_list( const config_option_t *option )
{
	assert(option != NULL);
 ...
 *         option
 */
static __inline unsigned int config_get_value_count( const config_option_t *option )
{
	assert(option != NULL);
	return option->values_count == 0 ?
