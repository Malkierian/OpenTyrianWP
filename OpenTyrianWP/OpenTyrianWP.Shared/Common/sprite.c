...
{
	char buffer[20];
	_snprintf(buffer, sizeof(buffer), "newsh%c.shp", tolower((unsigned char)s));
...
void JE_loadMainShapeTables( const char *shpfile )
{
	...
	JE_word shpNumb;
#ifdef TYRIAN2000
	JE_longint shpPos[14]; // 13 + 1 for storing file length
#else
	JE_longint shpPos[13]; // 12 + 1 for storing file length
#endif
