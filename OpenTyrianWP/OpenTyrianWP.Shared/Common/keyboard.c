...
{
	wait_input2(keyboard, mouse, joystick, true);
}

void wait_input2( JE_boolean keyboard, JE_boolean mouse, JE_boolean joystick, JE_boolean touch )
{
	service_SDL_events(false);
	while (!((keyboard && keydown) || (mouse && mousedown) || (joystick && joydown) || (touch && newtouch)))
	{
...
{
	wait_noinput2(keyboard, mouse, joystick, true);
}

void wait_noinput2( JE_boolean keyboard, JE_boolean mouse, JE_boolean joystick, JE_boolean touch )
{
	service_SDL_events(false);
	while ((keyboard && keydown) || (mouse && mousedown) || (joystick && joydown) || (touch && touchdown))
	{
		SDL_Delay(SDL_POLL_INTERVAL);
		poll_joysticks();
		service_SDL_events(false);
...
void init_keyboard( void )
{
	...
	newkey = newmouse = newtouch = false;
	keydown = mousedown = touchdown = false;
}
	...
	while (SDL_PollEvent(&ev))
	{
		switch (ev.type)
		{
			...
			case SDL_FINGERUP:
				newtouch = true;
				touchdown = false;
				touch_x = ev.tfinger.x;
				touch_y = ev.tfinger.y;
				normalizeCoordinates(&touch_x, &touch_y);
				DebugCoordinates(touch_x, touch_y, "Location", "");
				break;
