...
__inline static void blit_enemy( SDL_Surface *surface, unsigned int i, signed int x_offset, signed int y_offset, signed int sprite_offset );
...
enemy_still_exists:
			...
			if (enemy[i].launchfreq)
			{
				if (--enemy[i].launchwait == 0)
				{
					...
					{
						/*Type  1 : Must be __inline with player*/
			...
			do
			{
				...
				if (s[0] == ']')
				{
					...
					case 'Q':
						...
						{
							for (uint i = 0; i < 2; ++i)
								_snprintf(levelWarningText[i], sizeof(*levelWarningText), "%s %lu", miscText[40], player[i].cash);
...
bool JE_titleScreen( JE_boolean animate )
{
...
#endif
	{
		do
		{
			...
			{
				newtouch = false;
				menu = getMainTouchMenu(TYRIAN_MENU_MAIN);
				if (menu == TYRIAN_MENU_SELECT_NONE)
					newkey = false;
				if (menu == TYRIAN_MENU_KEYBOARD)
				{
					if (!SDL_HasScreenKeyboardSupport)
						Debug("Keyboard", "Screen keyboard not supported");
					else
					{
						SDL_StartTextInput();
						Debug("Keyboard", "Keyboard input started.");
					}
				}
			}
			...
			for (unsigned int i = 0; i < SA_ENGAGE; i++)
			{
				...
				if (arcade_code_i[i] > 0 && arcade_code_i[i] == strlen(specialName[i]))
				{
					...
					else if (i+1 == SA_ENGAGE)
					{
						...
						char buf[10+1+15+1];
						_snprintf(buf, sizeof(buf), "%s %s", miscTextB[4], pName[0]);
