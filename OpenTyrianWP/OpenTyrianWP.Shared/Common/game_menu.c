	...
	do
	{
			...
			for (int x = min; x <= max; x++)
			{
				...
				if (x < max) /* x == max isn't a save slot */
				{
					...
					else
					{
						...
						_snprintf(buf, sizeof buf, "%s%d", miscTextB[1-1], saveFiles[x-2].episode);
			...
			for (tempW = 1; tempW < menuChoices[curMenu]; tempW++)
			{
				...
				switch (curSel[1]-1)
				{
					case 1: /* ship */
						if (temp > 90)
						{
							_snprintf(tempStr, sizeof(tempStr), "Custom Ship %d", temp - 90);
						} else {
				...
				if (tempW != menuChoices[curMenu]-1)
				{
					...
					_snprintf(buf, sizeof buf, "Cost: %d", temp_cost);
		...
		if (((curMenu <= 2 || curMenu == 5 || curMenu == 6 || curMenu >= 10) && !twoPlayerMode) || (curMenu == 4 && (curSel[1] >= 1 && curSel[1] <= 6)))
		{
			if (curMenu != 4)
			{
				...
				_snprintf(buf, sizeof buf, "%lu", player[0].cash);
		...
		if (((curMenu >= 0 && curMenu <= 2) || curMenu == 5 || curMenu == 6 || curMenu >= 9) || (curMenu == 4 && (curSel[1] == 2 || curSel[1] == 5)))
		{
			if (twoPlayerMode)
			{
				...
				for (uint i = 0; i < 2; ++i)
				{
					_snprintf(buf, sizeof(buf), "%s %lu", miscText[40 + i], player[i].cash);
			...
			/* Inner loop -- this handles animations on menus that need them and handles
			   ...
			   Also, I think all timing is handled in here. Somehow. */
				...
				if (curMenu == 8)
				{
					...
					char buf[20];
					_snprintf(buf, sizeof(buf), "%s %d%%", miscText[11], percent_read);
...
JE_boolean JE_quitRequest( void )
{
	...
	while (!done)
	{
		...
		} while (!newkey && !mousedown && !newtouch);
