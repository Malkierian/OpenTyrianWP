...
#define MAX_PAGE 8
#define TOPICS 5
const JE_byte topicStart[TOPICS] = { 0, 1, 2, 3, 7 };
...
void JE_helpSystem( JE_byte startTopic )
{
	...
	do
	{
		...
		switch (page)
		{
			case 0:
				...
				do
				{
					...
					{
						newtouch = false;
						menu = getMainTouchMenu(TYRIAN_MENU_INSTRUCTIONS);
						if (menu != TYRIAN_MENU_SELECT_NONE)
						{
							newkey = true;
				...
				} while (!(lastkey_scan == SDL_SCANCODE_ESCAPE || lastkey_scan == SDL_SCANCODE_RETURN || lastkey_scan == SDL_SCANCODE_AC_BACK));
		...
		if (menu == 0)
		{
			do {
				...
				wait_delay();
			} while (!newkey && !newmouse && !newtouch);
	...
	} while (lastkey_scan != SDL_SCANCODE_ESCAPE && lastkey_scan != SDL_SCANCODE_AC_BACK);
}
...
void JE_loadScreen( void )
{
	...
	do
	{
		...
		if (newtouch)
		{
			...
			else if (touch_y > 0.15f)
			{
				if (touch_y < 0.2125)
				{
					sel = 1;
				}
				else if (touch_y < 0.275f)
				{
					sel = 2;
				}
				else if (touch_y < 0.3375f)
				{
					sel = 3;
				}
				else if (touch_y < 0.4f)
				{
					sel = 4;
				}
				else if (touch_y < 0.4625f)
				{
					sel = 5;
				}
				else if (touch_y < 0.525f)
				{
					sel = 6;
				}
				else if (touch_y < 0.5875f)
				{
					sel = 7;
				}
				else if (touch_y < 0.65f)
				{
					sel = 8;
				}
				else if (touch_y < 0.7125f)
				{
					sel = 9;
				}
				else if (touch_y < 0.775f)
				{
					sel = 10;
				}
				else if (touch_y < 0.8375f)
				{
					sel = 11;
				}
			}
...
bool load_next_demo( void )
{
	...
	_snprintf(demo_filename, sizeof(demo_filename), "demo.%d", demo_num);
...
void JE_endLevelAni( void )
{
	...
	if (twoPlayerMode)
	{
		for (uint i = 0; i < 2; ++i)
		{
			_snprintf(tempStr, sizeof(tempStr), "%s %lu", miscText[40 + i], player[i].cash);
...
void JE_inGameDisplays( void )
{
	...
	{
		_snprintf(tempstr, sizeof(tempstr), "%lu", player[i].cash);
