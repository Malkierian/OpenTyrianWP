	...
	do
	{
		...
		for (MenuOptions i = 0; i < MenuOptions_MAX; i++)
		{
			...
			/*if (i == MENU_FULLSCREEN)
			{
				...
				{
					_snprintf(buffer, sizeof(buffer), menu_items[i], temp_fullscreen_display + 1);
					text = buffer;
				}
			}
			else if (i == MENU_SCALER)
			{
				_snprintf(buffer, sizeof(buffer), "Scaler: %s", scalers[temp_scaler].name);
				text = buffer;
			}
			else if (i == MENU_SCALING_MODE)
			{
				_snprintf(buffer, sizeof(buffer), menu_items[i], scaling_mode_names[temp_scaling_mode]);
				text = buffer;
			}*/
		...
		{
			newtouch = false;
			sel = getMainTouchMenu(TYRIAN_MENU_OPENTYRIAN);
			if (sel != TYRIAN_MENU_SELECT_NONE)
			{
				newkey = true;
